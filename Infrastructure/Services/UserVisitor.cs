...
namespace BookingClinic.Infrastructure.Services
{
    public class UserVisitor : IUserVisitor, IDisposable
    {
        ...
        public void VisitPatient(VisitablePatientModel patient)
        {
            ...
            _lines.Add($"  Total Appointments: {patient.AppointmetsCount}");
            _lines.Add($"  Finished: {patient.FinishedAppointmentsCount}");
            _lines.Add($"  Canceled: {patient.CanceledAppointmentsCount}");
        }

        public void VisitAdmin(VisitableAdminModel admin)
        {
            if (admin == null) return;
            _lines.Add($"Admin: {admin.Name} {admin.Surname}");
        }

        public void VisitDoctor(VisitableDoctorModel doctor)
        {
            ...
            _lines.Add($"  Total Appointments: {doctor.AppointmetsCount}");
            _lines.Add($"  Finished: {doctor.FinishedAppointmentsCount}");
            _lines.Add($"  Canceled: {doctor.CanceledAppointmentsCount}");
        }
